 ...
 * @author paulcaplin
 */
public class PlayNew implements Screen {
    ...
    private void logic() {
        ...
        if (spawnCDEnemySlow <= 0f & enemyHandler.getNumEnemiesType(0) < 1) {
            enemyHandler.spawn(0);
            spawnCDEnemySlow = 3f;
        }
        if (spawnCDEnemyQuick <= 0f & enemyHandler.getNumEnemiesType(1) < 1) {
            enemyHandler.spawn(1);
            spawnCDEnemyQuick = 7f;
        }
        if (spawnCDEnemyStutterer <= 0f & enemyHandler.getNumEnemiesType(2) < 1) {
